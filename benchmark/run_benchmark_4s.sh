...
if [ "${token_path}" == "" ]; then
    Error "Please pass a value of original hugging face model using -tp or --token_path."
    exit 1
fi
if [ "${model_path}" == "" ]; then
    Error "Please pass a value of converted model path using -mp or --model_path."
    exit 1
fi
dtype=${dtype:-bf16}
...
Info "You are using model ${token_path}, dtype ${dtype}, batch size ${batch_size}, input tokens ${input_tokens}, output tokens ${output_tokens}, beam width ${beam_width} and iteration ${iter} on ${sockets} sockets system."
...
if [[ ${token_path} == *"llama"* ]] || [[ ${token_path} == *"baichuan"* ]]; then
...
elif [ "${numa_nodes}" -eq 4 ] && [ "$sockets" -eq 2 ]; then
...
elif [ "${numa_nodes}" -eq 4 ] && [ "$sockets" -eq 4 ]; then
    Info "4 socket SPR"
    export OMP_NUM_THREADS=$((${cores_per_numa} / 2))
    Info "OMP_NUM_THREADS: $((${cores_per_numa} / 2))"
    run_cmd="mpirun "
